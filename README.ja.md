...
<details>
<summary><b><code>rp</code> とは何か、そしてなぜこの名前なのか？</b></summary>

「repo」の短縮です。
短くする前は関数自体がその名前でしたし、内部のヘルパーは今もその名前を引き継いでいます（`_repo_hub_pick`、`_repo_open_dest` など）。

クローン済みのリポジトリに対してすることのすべてを、ひとつの入り口にまとめたものです。
`cd` する、パスを出力する、新しいものをクローンする、新規作成する、エディタで開く、パスをコピーする、削除する。
[ghq](https://github.com/x-motemen/ghq) と [fzf](https://github.com/junegunn/fzf) の上に置いた zsh 関数なので、選ぶ対象の一覧は `ghq list`、選ぶ操作そのものは fzf です。

どちらの向きでも使えます。
先に動作を指定してからリポジトリを選んでもいいですし（`rp cd -s foo`）、引数なしの `rp`（ハブモード）を実行して、リポジトリを先に選んでから何をするかをあとで決めてもかまいません。

結果をどこで開くかは、ほかの二つとは独立した三つめの選択です。
このシェル、tmux のセッションやウィンドウ、herdr のワークスペースやタブ、あるいはエディタから選べます。

</details>

<details>
<summary><b>zoxide や <code>ghq list | fzf</code> と何が違うのか？</b></summary>

`ghq` は、ディスク上のリポジトリがどこにあるかをすでに知っています。
`rp` は、そのリストに対して操作を加えるためのフロントエンドで、違いと呼べるものは三つあります。

- **ghq ネイティブ**：`ghq list` が唯一の情報源なので、たどり着ける先はクローン済みのものとちょうど一致します。温める必要のある frecency データベースもインポート作業もなく、新しいマシンでも `ghq get` を一度実行した時点で動きます。zoxide が答えるのは「どこに行ったことがあるか」、`rp` が答えるのは「何をクローンしてあるか」です。これは別の問いなので、両方を使うのは理にかなっています
...
たいていの人がすでに持っているのはそのためです。
「これをクローンして新しいワークスペースで開きたい」と思った時点で、操作 × 送り先のマトリクスを自分で書くことになります。
そのマトリクスが `rp` です。

</details>

<details>
<summary><b><code>rp</code> と <code>rp hub</code> は何が違うのか？</b></summary>

違いはありません。
`rp hub` は引数なしの `rp` を明示的に書いたものです。
ハブモードは既定の入り口であって、打たなければならないコマンドではありません。

</details>

<details>
<summary><b><code>-s</code> と <code>-w</code> は実際には何をするのか？</b></summary>

これらは操作ではなく送り先を指すので、`cd` でも `get` でも `create` でも同じ意味になります。
`-s` はセッションを開き、`-w` はウィンドウを開きます。
どのマルチプレクサになるかは自動で決まります。
`$HERDR_ENV` が設定されていれば [herdr](https://herdr.dev)、そうでなければ tmux です。
`-s` は同名のセッションやワークスペースがあれば再利用するので、重複が積み上がりません。
[マルチプレクサ連携](#マルチプレクサ連携)を参照してください。

</details>

<details>
<summary><b>クローンしていないリポジトリを選ぶとどうなるのか？</b></summary>

先にクローンされ、そのうえで選んだ動作が実行されます。
実際にこうなるのは `^G` を使ったときです。
`^G` は、ハブの一覧をローカルのリポジトリから GitHub のリポジトリに切り替えます。

</details>

<details>
<summary><b><code>rp open</code> はどのエディタを使うのか？</b></summary>

`rp open <editor>` → `$RP_EDITOR` → `$VISUAL` → `$EDITOR` → `code` → `vim` → `nvim` の順にたどり、最初に見つかったものです。
値には引数を付けられますし（`RP_EDITOR="code -n"`）、絶対パスでもかまいません。
...
<details>
<summary><b>ハブモードのキーは変更できるのか？</b></summary>

`enter` 以外のすべてのキーを、fzf のキー名を使って `RP_KEY_*` で変更できます。
これらはハブモードの起動時に検証され、`rp` が問題のある変数の名前を示すので、打ち間違いが fzf まで届くことはありません。
キーの割り当てを外す方法はありません。
代わりに手の届かないところへどかします。
`RP_KEY_REMOVE=f12` はそのためのものです。
[設定](#設定)を参照してください。

</details>

<details>
<summary><b>スクリプトの中で使えるのか？</b></summary>

`rp path` は `cd` する代わりにパスを出力するので、`cd "$(rp path dot)"` が動きます。
クエリを渡せば、ピッカーを開かずに解決します。
`rp path` と `rp cd` は `RP_KEY_*` を読まないので、キーの割り当てが壊れていても影響を受けません。

</details>

<details>
<summary><b>herdr の中でも動くのか？</b></summary>

動きます。
`-s` / `-w` は、`$HERDR_ENV` が設定されていれば、追加で何かをインストールしなくても herdr に解決されます。
そのうえで `rp` は herdr のプラグインとしても配布しているので、ひとつのキー割り当てでポップアップにハブモードが開き、そこからすべての操作に届きます。
[herdr プラグイン](#herdr-プラグイン)を参照してください。

</details>

<details>
<summary><b>なぜ <code>rp</code> は <code>PATH</code> 上のバイナリではなくシェル関数なのか？</b></summary>

`rp cd` が変えるのは、ほかでもない現在のシェルのディレクトリであり、子プロセスは親を `cd` できないからです。
アップグレードがすでに `rp` を実行したシェルに届かないのも、同じ理由です（下の項目を参照）。

</details>

<details>
<summary><b>zsh 専用なのか？ bash と fish はどうなのか？</b></summary>
...
fish は関数のモデルもクォートのモデルもまったく別で、さらに遠いところにあります。

やるとすれば、判断結果を出力するシェル非依存のコアを用意し、各シェルには `cd` だけの薄いラッパーを残す形になります。
ハブモードは内部ですでにその形になっていて、第一段階がトークンを出力し、呼び出し元がそれを受けて動きます。
その議論は [#10](https://github.com/peinan/rp/issues/10) でしています。

</details>

<details>
<summary><b>引数なしの <code>rp</code> が何も起きない、または <code>unknown option: --footer</code> と出る</b></summary>

`fzf` が 0.63.0 より古いということです。
ハブモードは `--footer` を使いますが、fzf は知らないオプションを無視せず致命的なエラーとして扱うので、呼び出すたびに失敗します。
今の `rp` は、代わりにそのことをそのまま伝えます。
制限がかかるのはハブモードだけです。
サブコマンドはそこまで新しいものを渡さないので、ずっと古い fzf でも使えます。

</details>

<details>
<summary><b>アップグレードしたのに何も変わらない</b></summary>
...
<details>
<summary><b>プレビューペインが空、またはエラーが出る</b></summary>

[`eza`](https://github.com/eza-community/eza) がない場合、プレビューは `ls -la` にフォールバックします。
`RP_PREVIEW_CMD` を設定しているなら、注意点が三つあります。
fzf はテンプレートを `$SHELL -c` で実行します。
`{}` は選択中の `host/user/repo`、`$RP_ROOT` は `$(ghq root)` で、クォートが必要です。
テンプレート中の `;;` は、ハブモードの `case` を早く閉じてしまいます。
[設定](#設定)を参照してください。
